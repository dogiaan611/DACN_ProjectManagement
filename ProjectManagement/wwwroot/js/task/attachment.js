...
function createSubtaskAttachmentHtml(attachment, boardId, columnId, taskId, subtaskId) {
    const isImage = /\.(jpg|jpeg|png|gif|webp)$/i.test(attachment.fileName);
    const isPptx = /\.(ppt|pptx)$/i.test(attachment.fileName);
    const isDocx = /\.(doc|docx)$/i.test(attachment.fileName);
    const isPdf = /\.(pdf)$/i.test(attachment.fileName);
    ...
    return `
        <div class="group flex items-center justify-between p-2 border rounded-md bg-white hover:bg-gray-50 text-sm text-gray-700 relative" id="attachment-${attachment.attachmentId}">
            <div class="flex items-center gap-2">
                <a href="/boards/${boardId}/columns/${columnId}/tasks/${taskId}/subtasks/${subtaskId}/attachments/${attachment.attachmentId}/download" target="_blank" class="flex items-center gap-2 hover:underline" title="${attachment.fileName}">
                    ${icon}
                <span class="truncate max-w-[150px]">${formatFileName(attachment.fileName)}</span>
                </a >
            </div >
...
export async function initSubtaskAttachments(taskId, subtaskId, boardId, columnId, projectId, containerElement) {
    const attachmentsContainer = containerElement.querySelector('#subtask-attachments-list');
    const uploadFileInput = containerElement.querySelector('#subtask-upload-file');
    let isListenerAttached = false;

    const renderAttachments = async () => {
        const subtaskAttachments = await getSubtaskAttachments(taskId, subtaskId, boardId, columnId);
        attachmentsContainer.innerHTML = subtaskAttachments.map(attachment => createSubtaskAttachmentHtml(attachment, boardId, columnId, taskId, subtaskId)).join('');
    };
    ...
    const handleContainerClick = async (e) => {
        ...
        if (deleteBtn) {
            ...
            if (confirm('Are you sure you want to delete this attachment?')) {
                const attachmentId = deleteBtn.dataset.attachmentId;
                try {
                    await deleteSubtaskAttachment(boardId, columnId, taskId, subtaskId, attachmentId);
                    await renderAttachments();
                } catch (err) {
                    alert('Failed to delete attachment');
                }
    ...
    newUploadInput.addEventListener('change', async (e) => {
        ...
        if (file) {
            try {
                await addSubtaskAttachments(boardId, columnId, taskId, subtaskId, file);
                await renderAttachments();
                newUploadInput.value = '';
            } catch (err) {
                alert('Failed to upload attachment: ' + err.message);
